...
class Generator(nn.Generator):
    ...
    def __init__(self, sess):
        ...
        with tf.variable_scope('gen'):
            ...
            self.input_seed_ = tf.placeholder(tf.float32, shape=[None, 100], name='input_seed')
            self.concat = tf.concat([self.input_state_, self.input_seed_], 1, name='concat')
            self.hidden = tf.layers.dense(self.concat, 128, activation=tf.nn.tanh, name='hidden')
...
class Discriminator_copy(nn.Discriminator_copy):
    ...
    def __init__(self, dis, new_rew_input):
        ...
        self.output_ = tf.matmul(self.hidden, self.output_ker) + self.output_bias
